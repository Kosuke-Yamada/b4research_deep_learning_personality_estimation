...
READ_TEST_TWEET_FILE = "../DATA/09.DATASET_TDT/test_tweet_word_count.json"
TEST_LIST = [READ_TEST_MBTI_FILE, READ_TEST_TWEET_FILE]

READ_MODEL_FILE = "../DATA/11.MODEL2/save_model/"+str(MBTI)+"_"+str(NUM).zfill(2)+".json"
READ_WEIGHT_BEST_FILE = "../DATA/11.MODEL2/best_weight/"+str(MBTI)+"_"+str(NUM).zfill(2)+".hdf5"

WRITE_ROC_PICTURE_FILE = "../DATA/11.MODEL2/test_roc_picture/"+str(MBTI)+"_"+str(NUM).zfill(2)+".png"
...
def print_sample(xtest, ytest, model, max_features, num_sample):
    for i in range(num_sample):
        num_random = np.random.randint(len(xtest))
        xdata = xtest[num_random].reshape(1, max_features)
...
#ROC,AUCの出力
def print_roc_auc(xtest, ytest, model, max_features, roc_picture_file):
...
if __name__ == '__main__':
    ...
    xtest, ytest = make_feature_vector(TEST_LIST, num_test_data, MBTI, max_features)
    ...
    #サンプル出力
    print_sample(xtest, ytest, model, max_features, NUM_SAMPLE)
    
    #混合行列出力
    print_confusion_matrix(xtest, ytest, model, max_features)

    #ROC出力
    print_roc_auc(xtest, ytest, model, max_features, WRITE_ROC_PICTURE_FILE)
