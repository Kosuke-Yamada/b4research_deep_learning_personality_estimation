...
#特徴ベクトル作成
def make_feature_vector(read_file_list, num_data, mbti, max_sentence_length):
...
#混合行列のための計算
def print_confusion_matrix(xtest, ytest, model, max_sentence_length):
    tp = 0
    fn = 0
    fp = 0
    tn = 0
...
#ROC,AUCの出力
def print_roc_auc(xtest, ytest, model, max_sentence_length, roc_picture_file):
...
if __name__ == '__main__':
    ...
    xtest, ytest = make_feature_vector(TEST_LIST, num_test_data, MBTI, MAX_SENTENCE_LENGTH)
    ...
    #サンプル出力
    print_sample(xtest, ytest, model, MAX_SENTENCE_LENGTH, NUM_SAMPLE)
    
    #混合行列出力
    print_confusion_matrix(xtest, ytest, model, MAX_SENTENCE_LENGTH)

    #ROC出力
    print_roc_auc(xtest, ytest, model, MAX_SENTENCE_LENGTH, WRITE_ROC_PICTURE_FILE)
