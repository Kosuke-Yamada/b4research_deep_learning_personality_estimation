...
READ_TEST_TWEET_FILE = "../DATA/09.DATASET_TDT/test_tweet_index_list.json"
TEST_LIST = [READ_TEST_MBTI_FILE, READ_TEST_TWEET_FILE]

READ_MODEL_FILE = "../DATA/11.MODEL4/save_model/"+str(MBTI)+"_"+str(NUM).zfill(2)+".json"
READ_WEIGHT_BEST_FILE = "../DATA/11.MODEL4/best_weight/"+str(MBTI)+"_"+str(NUM).zfill(2)+".hdf5"

WRITE_ROC_PICTURE_FILE = "../DATA/11.MODEL4/test_roc_picture/"+str(MBTI)+"_"+str(NUM).zfill(2)+".png"
...
def print_sample(xtest, ytest, model, user_count_tweet, len_tweet, num_sample):
    for i in range(num_sample):
        num_random = np.random.randint(len(xtest))
        xdata = xtest[num_random].reshape(1, user_count_tweet, len_tweet)
        ylabel = ytest[num_random]
        ypred = model.predict(xdata)[0][0]
        print(str(ypred)+"\t"+str(ylabel)+"\n"+str(xdata))

#混合行列のための計算
def print_confusion_matrix(xtest, ytest, model, user_count_tweet, len_tweet):
    tp = 0
    fn = 0
    fp = 0
    tn = 0
    for i in range(len(xtest)):
        xdata = xtest[i].reshape(1, user_count_tweet, len_tweet)
        ylabel = int(ytest[i])
        ypred = model.predict(xdata)[0][0]
...
#ROC,AUCの出力
def print_roc_auc(xtest, ytest, model, user_count_tweet, len_tweet, roc_picture_file):
...
if __name__ == '__main__':
    ...
    xtest, ytest = make_feature_vector(TEST_LIST, num_test_data, MBTI, USER_COUNT_TWEET, LEN_TWEET)
    ...
    #サンプル出力
    print_sample(xtest, ytest, model, USER_COUNT_TWEET, LEN_TWEET, NUM_SAMPLE)
    
    #混合行列出力
    print_confusion_matrix(xtest, ytest, model, USER_COUNT_TWEET, LEN_TWEET)

    #ROC出力
    print_roc_auc(xtest, ytest, model, WRITE_ROC_PICTURE_FILE)
