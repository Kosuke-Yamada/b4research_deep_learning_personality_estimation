...
from keras.layers import Embedding, Lambda, Dropout, Dense, Activation
...
READ_TRAIN_TWEET_FILE = "../DATA/09.DATASET_TDT/train_tweet_word_binary.json"
TRAIN_LIST = [READ_TRAIN_MBTI_FILE, READ_TRAIN_TWEET_FILE]
READ_DEV_MBTI_FILE = "../DATA/09.DATASET_TDT/dev_mbti.json"
READ_DEV_TWEET_FILE = "../DATA/09.DATASET_TDT/dev_tweet_word_binary.json"
DEV_LIST = [READ_DEV_MBTI_FILE, READ_DEV_TWEET_FILE]

WRITE_DIR = "../DATA/11.MODEL1/"
...
#特徴ベクトル作成
def make_feature_vector(read_file_list, num_data, mbti, max_features):
...
#モデル作成
def make_model(hidden_layer_size, max_features, dropout_rate):
    model = Sequential()
    model.add(Dense(hidden_layer_size, input_dim = max_features, activation = "relu"))
...
#混合行列のための計算
def print_confusion_matrix(xdev, ydev, model, max_features):
    tp = 0
    fn = 0
    fp = 0
    tn = 0
    for i in range(len(xdev)):
        xdata = xdev[i].reshape(1, max_features)
        ylabel = int(ydev[i])
...
#ROC,AUCの出力
def print_roc_auc(xdev, ydev, model, max_features):
...
if __name__ == '__main__':
    ...
    xtrain, ytrain = make_feature_vector(TRAIN_LIST, num_train_data, MBTI, max_features)
    xdev, ydev = make_feature_vector(DEV_LIST, num_dev_data, MBTI, max_features)
    print(xtrain.shape, ytrain.shape, xdev.shape, ydev.shape)

    #モデルの作成
    model = make_model(HIDDEN_LAYER_SIZE, max_features, DROPOUT_RATE)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        #混合行列出力
        print_confusion_matrix(xdev, ydev, model, max_features)

        #ROC出力
        AUC = print_roc_auc(xdev, ydev, model, max_features)
