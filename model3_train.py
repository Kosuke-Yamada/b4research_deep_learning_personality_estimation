...
#混合行列のための計算
def print_confusion_matrix(xdev, ydev, model, max_sentence_length):
    tp = 0
    fn = 0
    fp = 0
    tn = 0
...
#ROC,AUCの出力
def print_roc_auc(xdev, ydev, model, max_sentence_length):
...
if __name__ == '__main__':
    ...
    xtrain, ytrain = make_feature_vector(TRAIN_LIST, num_train_data, MBTI, MAX_SENTENCE_LENGTH)
    xdev, ydev = make_feature_vector(DEV_LIST, num_dev_data, MBTI, MAX_SENTENCE_LENGTH)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        #混合行列出力
        print_confusion_matrix(xdev, ydev, model, MAX_SENTENCE_LENGTH)

        #ROC出力
        AUC = print_roc_auc(xdev, ydev, model, MAX_SENTENCE_LENGTH)
