...
TRAIN_LIST = [READ_TRAIN_MBTI_FILE, READ_TRAIN_TWEET_FILE, READ_TRAIN_DES_FILE]
...
DEV_LIST = [READ_DEV_MBTI_FILE, READ_DEV_TWEET_FILE, READ_DEV_DES_FILE]

WRITE_DIR = "../DATA/11.MODEL3-2/"
...
#モデル作成
def make_model(embedding_size, max_tweet_length, dropout_rate, hidden_layer_size, max_des_length):
    ...
    mean = concatlayer([mean1, mean2])
    den = denselayer(mean)
    drop = dropoutlayer(den)
    den2 = dense2layer(drop)
    y = activationlayer(den2)
    
    model = Model(inputs = [x_tweet, x_des], outputs = y)
    return model

#混合行列のための計算
def print_confusion_matrix(xdev_tweet, xdev_des, ydev, model, max_tweet_length, max_des_length):
    tp = 0
    fn = 0
    fp = 0
    tn = 0
    for i in range(len(xdev_tweet)):
        x_tweet = xdev_tweet[i].reshape(1, max_tweet_length)
        x_des = xdev_des[i].reshape(1, max_des_length)
        ylabel = int(ydev[i])
...
#ROC,AUCの出力
def print_roc_auc(xdev_tweet, xdev_des, ydev, model, max_tweet_length, max_des_length):
...
if __name__ == '__main__':
    ...
    xtrain_tweet, xtrain_des, ytrain = make_feature_vector(TRAIN_LIST, num_train_data, MBTI, MAX_TWEET_LENGTH, MAX_DES_LENGTH)
    xdev_tweet, xdev_des, ydev = make_feature_vector(DEV_LIST, num_dev_data, MBTI, MAX_TWEET_LENGTH, MAX_DES_LENGTH)
    print(xtrain_tweet.shape, xtrain_des.shape, ytrain.shape, xdev_tweet.shape, xdev_des.shape, ydev.shape)

    #モデルの作成
    model = make_model(EMBEDDING_SIZE, MAX_TWEET_LENGTH, DROPOUT_RATE, HIDDEN_LAYER_SIZE, MAX_DES_LENGTH)
        
    model.compile(loss = "binary_crossentropy", optimizer = "adam", metrics = ["accuracy"])
    model.summary()
    plot_model(model, to_file = WRITE_MODEL_PICTURE_FILE, show_shapes = True)
    history = model.fit([xtrain_tweet, xtrain_des],
                        ytrain,
                        ...
                        validation_data = ([xdev_tweet, xdev_des], ydev),
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        #混合行列出力
        print_confusion_matrix(xdev_tweet, xdev_des, ydev, model, MAX_TWEET_LENGTH, MAX_DES_LENGTH)

        #ROC出力
        AUC = print_roc_auc(xdev_tweet, xdev_des, ydev, model, MAX_TWEET_LENGTH, MAX_DES_LENGTH)
